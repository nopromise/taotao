 ...
 * 查询商品实体对象Service
 * 
 * @author fanjunlin 知识点：redis缓存
 *         缓存过期的时间：如果用户访问了一个特别冷门的商品，一个月也就访问一两次，浪费内存。热门商品设置商品的过期时期，也没问题。
 */
@Service
public class ItemServiceImpl implements ItemService {
	...
	public TbItem getItemById(Long itemId) {
		...
		try {
			//
			// 对象item是不能直接保存到redis中的，需要转出json
	...
	public TbItemDesc getItemDescById(Long itemId) {
		try {
			...
			// 判断数据是否存在
			if (StringUtils.isNotBlank(json)) {
				// System.out.println("TbItemDesc：从缓存中取得数据");
				// 把json数据转换成java对象
		...
		try {
			...
			// System.out.println("TbItemDesc：往缓存中放入数据");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return itemDesc;
