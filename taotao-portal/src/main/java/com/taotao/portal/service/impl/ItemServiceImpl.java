...
public class ItemServiceImpl implements ItemService {
	...
	@Value("${REST_BASE_URL}")
	private String REST_BASE_URL;// http://localhost:8081/rest

	@Value("${REST_ITEM_BASE_URL}")
	private String REST_ITEM_BASE_URL;// /item/base

	@Value("${REST_ITEM_DESC_URL}")
	private String REST_ITEM_DESC_URL;// /item/desc/
	
	@Value("${REST_ITEM_PARAM_URL}")
	private String REST_ITEM_PARAM_URL;// /item/param/

	// 根据商品id查询商品基本信息
	public TbItem getItemById(Long itemId) {
		// http://localhost:8081/rest/item/base/151557216918024
		String json = HttpClientUtil.doGet(REST_BASE_URL + REST_ITEM_BASE_URL + itemId);
		// 转换成java对象【json是TaotaoResult类的格式数据】
